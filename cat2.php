...
$query = mysqli_query($conn, "SELECT * FROM parameters WHERE category_id = 2 ORDER BY parameter_id ASC");
$params = [];
...
<head>
  ...
  <style>
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
    ...
    .table-head .col-copy-head { /* empty spacer */ }
...
<div class="container">
  ...
  <h1 class="title">Category 2 — Candidate Details</h1>
  ...
  <form method="POST" id="catForm">
    ...
    <?php foreach ($params as $i => $row):
      ...
      $saved_current  = $_SESSION['cat2']['current'][$pid]  ?? '';
      $saved_previous = $_SESSION['cat2']['previous'][$pid] ?? '';
    ?>
    <div class="table-row">
      ...
      <div class="stepper" id="stepper-c-<?php echo $pid; ?>">
        ...
        <input type="number"
               ...
               placeholder="0"
               onkeydown="handleKey(event, <?php echo $i; ?>, 'current')">
        <button type="button" onclick="step('current[<?php echo $pid; ?>]', 1)">+</button>
      </div>

      <!-- Previous Year stepper -->
      <div class="stepper" id="stepper-p-<?php echo $pid; ?>">
        ...
        <input type="number"
               ...
               placeholder="0"
               onkeydown="handleKey(event, <?php echo $i; ?>, 'previous')">
        <button type="button" onclick="step('previous[<?php echo $pid; ?>]', 1)">+</button>
      </div>
    ...
    <div class="btn-row">
      <button type="button" class="btn btn-back" onclick="window.location.href='cat1.php'">Back</button>
...
<script>
  // All parameter IDs for bulk ops
  const pids = [<?php echo implode(',', array_column($params, 'parameter_id')); ?>];

  // Stepper: increment/decrement by name attribute
  ...
  // Copy current → previous for a row
  function copyToPrev(pid) {
    const cur = document.getElementById('current_' + pid);
    const prev = document.getElementById('previous_' + pid);
    if (cur && prev) {
      prev.value = cur.value;
      // Flash feedback
      prev.closest('.stepper').style.borderColor = '#16a34a';
      setTimeout(() => prev.closest('.stepper').style.borderColor = '', 600);
    }
  }

  // Set ALL fields to a value
  function setAll(val) {
    pids.forEach(pid => {
